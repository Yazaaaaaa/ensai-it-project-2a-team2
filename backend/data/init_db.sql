...
CREATE TABLE NEOW.neo (
    ...
    is_custom           BOOLEAN,
    created_by_user_id  INT REFERENCES NEOW.users(id_user) -- Correction ici
);

CREATE TABLE NEOW.login_history (
    id_login    SERIAL PRIMARY KEY,
    id_user     INT REFERENCES NEOW.users(id_user),    -- Correction ici
...
CREATE TABLE NEOW.alert (
    ...
    id_user           INT REFERENCES NEOW.users(id_user), -- Correction ici
    id_neo            INT REFERENCES NEOW.neo(id_neo),    -- Correction ici
    min_size_m        FLOAT,
...
CREATE TABLE NEOW.notification (
    ...
    id_user         INT REFERENCES NEOW.users(id_user),   -- Correction ici
    id_alert        INT REFERENCES NEOW.alert(id_alert),  -- Correction ici
    message         TEXT,
...
CREATE TABLE NEOW.distance_history (
    ...
    id_favorite         INT REFERENCES NEOW.favorite(id_favorite), -- Correction ici
    recorded_at         DATE,
    distance_km         FLOAT
);
